...
export default {
  ...
  methods: {
    ...
    async cancelAppointment(doctorId, visitTimeId, isCancel = true) {
      try {
        const response = await makeAppointment({ doctorId, visitTimeId, isCancel }) // 使用相同的接口来取消预约
        if (response.code === 20000) {
          this.$message.success('取消预约成功！')
          // 更新排班状态
          const doctor = this.doctors.find((d) => d.id === doctorId)
          const schedule = doctor.schedule.find((s) => s.visitTimeId === visitTimeId)
          schedule.isReserved = false // 取消预约标记为未预约
          schedule.slotCount += 1 // 增加号源
        } else {
